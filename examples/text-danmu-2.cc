// Vector text overlay: danmaku example 2 - dense fast lanes.
//
// Small (~28 px) bright white/yellow bullets moving fast (~3.5 s) through
// tightly packed lanes, the high-density effect from the reference brief
// (0:31-0:32).
//
// Usage:
//   text-danmu-2.exe --demo --out DIR
//   text-danmu-2.exe --help
//
...
make_document()
{
  ...
  constexpr double lane_h = 150.0;
  constexpr double area_top = 240.0;
  ...
  doc.width = 1080.0;
  doc.height = 1920.0;
  doc.area_percent = 62.0;   // bulletchatlist.area
  doc.allow_overlap = false; // bulletchatlist.allowOverlap
  doc.seed = "danmaku-20260814-2";
  // Sixteen short runs across eight lanes, alternating bright white/yellow.
  add_run(doc, "danmu-2-01", "IZZI", 0, 0.0, 0.0, "#FFFFFF");
  add_run(doc, "danmu-2-02", "KOKI", 1, 0.2, 0.0, "#FFE94A");
  add_run(doc, "danmu-2-03", "ZOI", 2, 0.4, -0.8, "#FFFFFF");
  add_run(doc, "danmu-2-04", "2026", 3, 0.6, 0.0, "#FFD12E");
  add_run(doc, "danmu-2-05", "111", 4, 0.8, -1.2, "#FFFFFF");
  add_run(doc, "danmu-2-06", "IZ", 5, 1.0, 0.0, "#FFE94A");
  add_run(doc, "danmu-2-07", "OK", 6, 1.2, -0.5, "#FFFFFF");
  add_run(doc, "danmu-2-08", "KO", 7, 1.4, 0.0, "#FFD12E");
  add_run(doc, "danmu-2-09", "ZOI", 0, 1.6, 0.0, "#FFE94A");
  add_run(doc, "danmu-2-10", "000", 1, 1.8, -1.0, "#FFFFFF");
  add_run(doc, "danmu-2-11", "I0", 2, 2.0, 0.0, "#FFD12E");
  add_run(doc, "danmu-2-12", "Z2", 3, 2.2, -1.4, "#FFFFFF");
  add_run(doc, "danmu-2-13", "O1", 4, 2.4, 0.0, "#FFE94A");
  add_run(doc, "danmu-2-14", "K2", 5, 2.6, -0.7, "#FFFFFF");
  add_run(doc, "danmu-2-15", "IZ2", 6, 2.8, 0.0, "#FFD12E");
  add_run(doc, "danmu-2-16", "K01", 7, 3.0, -1.1, "#FFFFFF");
  return doc;
}
...
run_determinism_check(const document& doc)
{
  ...
  std::cout << "[text-danmu-2] determinism: "
...
{
  std::cout
    << "usage: text-danmu-2.exe [--demo --out DIR]\n"
    << "                       [--help]\n";
}
...
main(int argc, char** argv)
{
  try
      ...
      if (!demo)
	{
      ...
      std::cout << "[text-danmu-2] wrote " << svg_path.string() << " and "
		<< manifest_path.string() << '\n';
      return run_determinism_check(doc);
    }
  catch (const std::exception& error)
    {
      std::cerr << "text-danmu-2: " << error.what() << '\n';
      return 1;
    }
}
