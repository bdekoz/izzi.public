// Vector text overlay: danmaku example 3 - large slow titles.
//
// Large (~72 px) saturated-color bullets moving slowly (~13 s) with dark
// strokes and rounded background boxes, the accent-title effect from the
// reference brief (0:31-0:32).
//
// Usage:
//   text-danmu-3.exe --demo --out DIR
//   text-danmu-3.exe --help
//
...
make_document()
{
  ...
  constexpr double lane_h = 260.0;
  constexpr double area_top = 320.0;

  auto add_run = [](document& doc, const std::string& id,
		    const std::string& source, const int lane,
		    const double start, const std::string& fill)
    {
      constexpr double em = 72.0;
      run r;
      r.id = id;
      r.source = source;
      r.m = mode::scroll;
      ...
      r.stroke_width = 5.0;
      r.background = "#0B0E14";
      r.background_opacity = 0.55;
      r.padding = 14.0;
      r.radius = 16.0;
      r.shaper = "builtin-stroke-set";
      r.shaper_version = "danmu-3";
      double x = 0.0;
  ...
  doc.width = 1080.0;
  doc.height = 1920.0;
  doc.area_percent = 55.0;   // bulletchatlist.area
  doc.allow_overlap = false; // bulletchatlist.allowOverlap
  doc.seed = "danmaku-20260814-3";
  add_run(doc, "danmu-3-01", "IZZI", 0, 0.0, "#22F0FF");
  add_run(doc, "danmu-3-02", "KOKI", 1, 1.5, "#FF1F8F");
  add_run(doc, "danmu-3-03", "2026", 2, 3.0, "#FFC400");
  add_run(doc, "danmu-3-04", "ZOI", 3, 4.5, "#A85CFF");
  add_run(doc, "danmu-3-05", "OKI", 4, 6.0, "#F7F4EB");
  add_run(doc, "danmu-3-06", "111", 2, 7.5, "#1867E5");
  return doc;
}
...
run_determinism_check(const document& doc)
{
  ...
  std::cout << "[text-danmu-3] determinism: "
...
{
  std::cout
    << "usage: text-danmu-3.exe [--demo --out DIR]\n"
    << "                       [--help]\n";
}
...
main(int argc, char** argv)
{
  try
      ...
      if (!demo)
	{
      ...
      std::cout << "[text-danmu-3] wrote " << svg_path.string() << " and "
		<< manifest_path.string() << '\n';
      return run_determinism_check(doc);
    }
  catch (const std::exception& error)
    {
      std::cerr << "text-danmu-3: " << error.what() << '\n';
      return 1;
    }
}
